...
void updateSolenoid() {
  ...
  // Check EM1 timing
  if (em1_active && (now - em1_trigger_time >= PULSE_DURATION_MS)) {
    analogWrite(EM1_PIN, 0);
    em1_active = false;
  ...
  // Check EM2 timing
  if (em2_active && (now - em2_trigger_time >= PULSE_DURATION_MS)) {
    analogWrite(EM2_PIN, 0);
    em2_active = false;
  ...
  // Check EM3 timing
  if (em3_active && (now - em3_trigger_time >= PULSE_DURATION_MS)) {
    analogWrite(EM3_PIN, 0);
    em3_active = false;
  ...
  if (sol_active && (now - sol_trigger_time >= PULSE_DURATION_MS)) {
    analogWrite(SOLENOID_PIN, 0);
    sol_active = false;
